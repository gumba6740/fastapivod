...
class TestMeetingRouter(TestCase):
    ...
    async def test_api_update_meeting_date_range_mysql(self) -> None:
        async with httpx.AsyncClient(
            ...
            response_body = response.json()
            self.assertEqual(response_body["url_code"], url_code)
            self.assertEqual(response_body["start_date"], str(today))
            self.assertEqual(response_body["end_date"], str(today))
            meeting = await MeetingModel.filter(url_code=url_code).get()
            self.assertEqual(meeting.start_date, today)
            self.assertEqual(meeting.end_date, today)

        # reveal_type(meeting.start_date)
        # reveal_type(today)
